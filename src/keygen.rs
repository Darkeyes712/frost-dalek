...
impl SecretShare {
    /// Custom serialization method for SecretShare.
    /// This avoids exposing sensitive information while still allowing it to be saved.
    pub fn serialize<W: Write>(&self, writer: &mut W) -> std::io::Result<()> {
        // Serialize the participant index (u32)
        writer.write_all(&self.index.to_le_bytes())?;

        // Serialize the polynomial evaluation (as a byte array) securely
        writer.write_all(&self.polynomial_evaluation.to_bytes())?;

        Ok(())
    }

    /// Custom deserialization method for SecretShare.
    /// This reconstructs the object from serialized data without exposing sensitive fields.
    pub fn deserialize<R: Read>(reader: &mut R) -> std::io::Result<SecretShare> {
        // Deserialize the participant index (u32)
        let mut index_bytes = [0u8; 4];
        reader.read_exact(&mut index_bytes)?;
        let index = u32::from_le_bytes(index_bytes);

        // Deserialize the polynomial evaluation (Scalar)
        let mut scalar_bytes = [0u8; 32];
        reader.read_exact(&mut scalar_bytes)?;
        let polynomial_evaluation = Scalar::from_bytes_mod_order(scalar_bytes);

        // Return the reconstructed SecretShare
        Ok(SecretShare {
            index,
